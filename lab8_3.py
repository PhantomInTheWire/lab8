...
def plot_results(V, policy, filename_prefix="gbike_modified"):
    ...
    sns.heatmap(policy, annot=True, fmt="d", cmap="RdBu_r", cbar=True,
                ...
                vmin=-5, vmax=5)
    ax.invert_yaxis()
    plt.title("Optimal Policy: Net Bike Transfers (Modified Problem)", 
    ...
    ax.invert_yaxis()
    plt.title("Value Function: Expected Return (Modified Problem)", 
              fontsize=14, fontweight='bold', pad=20)
    plt.xlabel("Bikes at Location 2 (End of Day)", fontsize=12, fontweight='bold')
    plt.ylabel("Bikes at Location 1 (End of Day)", fontsize=12, fontweight='bold')
    plt.tight_layout()
    ...
    ax.set_title('Value Function: 3D Surface (Modified Problem)', fontsize=14, fontweight='bold', pad=20)
...
if __name__ == "__main__":
    print("Solving Modified Gbike Rental Problem...")
    print("Modifications:")
    print("  - Free shuttle: 1 bike from Location 1 to Location 2")
    print("  - Parking cost: INR 4 if > 10 bikes at a location")
    print()
    gbike = GbikeRentalModified()
